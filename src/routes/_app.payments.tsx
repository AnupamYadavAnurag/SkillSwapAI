...
const PACKS = [
  ...
  { coins: 500, price: 19.99, popular: true },
  { coins: 1200, price: 39.99 },
];
...
function PaymentsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader title="Payments & Withdrawals" subtitle="Top up your balance or cash out your coins." />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {PACKS.map((p) => (
              <motion.div key={p.coins} whileHover={{ y: -4 }}>
                <Card className={`relative cursor-pointer p-4 text-center transition-shadow hover:shadow-elegant ${p.popular ? "border-primary shadow-glow" : ""}`}>
                  {p.popular && (
                  ...
                  <Button size="sm" onClick={() => buy(p)} className="mt-3 w-full bg-gradient-primary text-primary-foreground">
                    Buy
                  </Button>
                </Card>
              </motion.div>
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-display text-xl font-bold">Withdraw to bank / UPI</h2>
          </div>
          <p className="mb-4 text-sm text-muted-foreground">Available: <span className="font-bold text-foreground">{coins} coins</span> (≈ ${(coins * 0.05).toFixed(2)})</p>
          <form onSubmit={withdraw} className="space-y-4">
            <div className="space-y-1.5">
              <Label htmlFor="upi">UPI / Bank account</Label>
              <Input id="upi" placeholder="alex@upi or IBAN…" value={upi} onChange={(e) => setUpi(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="amt">Amount ({withdrawAmount} coins ≈ ${(withdrawAmount * 0.05).toFixed(2)})</Label>
              <input
                id="amt"
                type="range"
                min={50}
