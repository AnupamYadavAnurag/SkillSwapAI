...
function AuthPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    toast.success(mode === "signup" ? "Welcome to SkillSwapAI!" : "Welcome back!");
    nav({ to: "/dashboard" });
  };

  const social = (provider: string) => {
    login(`${provider.toLowerCase()}@skillswap.ai`);
      ...
      >
        <div className="mb-6 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {mode === "login" ? "Log in to continue learning & earning" : "Join thousands of learners worldwide"}
            </p>
          </div>
        </div>
        ...
        <div className="mb-5 grid grid-cols-2 gap-2">
          <Button variant="outline" type="button" onClick={() => social("Google")}>
            <svg className="h-4 w-4" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.5 12.27c0-.78-.07-1.53-.2-2.27H12v4.51h5.92a5.07 5.07 0 0 1-2.2 3.32v2.77h3.55c2.08-1.92 3.23-4.74 3.23-8.33Z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.27-2.66l-3.55-2.77c-.98.66-2.24 1.05-3.72 1.05-2.86 0-5.29-1.93-6.16-4.53H2.18v2.85A11 11 0 0 0 12 23Z"/><path fill="#FBBC05" d="M5.84 14.09a6.6 6.6 0 0 1 0-4.18V7.06H2.18a11 11 0 0 0 0 9.88l3.66-2.85Z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.2 1.64l3.15-3.15A11 11 0 0 0 2.18 7.06l3.66 2.85C6.71 7.31 9.14 5.38 12 5.38Z"/></svg>
            Google
          </Button>
